...
export default function CreateCampaignPage() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // 🔹 Immediately fetch updated campaigns
      fetchCampaigns();
    } else {
          ...
          <div className="bg-gray-900/80 backdrop-blur-md p-8 rounded-2xl shadow-2xl transform transition-transform duration-500 hover:rotate-y-6">
            {!saved && (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
  type="submit"
  className="w-full py-3 px-4 font-semibold rounded-lg shadow-md bg-indigo-600 hover:bg-indigo-700 transition-all duration-300 transform hover:scale-105 cursor-pointer text-white"
>
  {status === "Saving..." ? "Saving..." : saved ? "Saved" : "Save Campaign"}
</button>
              </form>
